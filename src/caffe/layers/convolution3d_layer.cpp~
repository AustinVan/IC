...
Dtype Convolution3DLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const int tempdata=bottom[0]->count();
  LOG(INFO)<<"input total:  "<<tempdata;
  ...
  const int tempout=(*top)[0]->count();
  forward_timer.Stop();
  LOG(INFO)<<"forward time:"<<forward_timer.MilliSeconds()<<" ms";
  LOG(INFO)<<"output total: "<< tempout;
  LOG(INFO)<<"number:"<<num_<<"  out_length*out_height*out_withth :"<<
	  (length_+2*temporal_pad_-kernel_depth_)/temporal_stride_+1<<"*"<<
	  (height_+2*pad_-kernel_size_)/stride_+1<<"*"<<(width_+2*pad_-kernel_size_)/stride_+1;
  LOG(INFO)<<"filter_gruop_:  "<<filter_group_<<"  input c*l*h*w:  "<<channels_<<
